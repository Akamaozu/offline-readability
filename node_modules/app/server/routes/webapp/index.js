...
// routes
  ...
  // list favorites
    webapp_routes.get('/favorites/:page?', function( req, res ){
      ...
      readability.bookmarks( req.session.auth.oauth_token, req.session.auth.oauth_token_secret, { page: page, favorite: 1 }, function( error, response ){
  ...
  // list archives
    webapp_routes.get('/archives/:page?', function( req, res ){
      ...
      readability.bookmarks( req.session.auth.oauth_token, req.session.auth.oauth_token_secret, { page: page, archive: 1 }, function( error, response ){
  ...
  // api token
    webapp_routes.get('/api-token', function( req, res ){

      if( !req.session.auth ) return res.sendStatus( 401 );
      
      var signed_tokens = require('jwt-simple').encode( req.session.auth, process.env.JWT_USER_TOKEN_SECRET );
